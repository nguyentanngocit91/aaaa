  ...
  nangcapHD({required dynamic data}) async {
    ...
    try {
      ...
      final customer = await App.dioClient
          .get("${ApiUrl.infoUpdateCustomer}${data['idkhachhang']}");
      final tmp = data;
      ...
      if (customer.statusCode == 200) {
        if (customer.data['success'] == false) {
          ...
          final dataInsertHD = tmp[insertMap[data['type']]];

          dataInsertHD['loaihopdong'] = data['type'];
          ...
          if (response.statusCode == 200) {
            if (response.data['success'] == true) {
              ...
              [
                ...
                dataInsertPT[e] = int.tryParse(dataInsertPT[e]
                    .toString()
                    .replaceAll(RegExp(r'[^0-9]'), ''));
              });
              ...
              resultMessenge.add("Không thêm được hợp đồng (${response.data['message']})");
      ...
      resultResponse['msg'] = ["Lỗi hệ thống 1 ${exception.toString()}"];
      resultResponse['status'] = false;
    } catch (error) {

      resultResponse['msg'] = ["Lỗi hệ thống 2x ${error.toString()}"];
      resultResponse['status'] = false;
    }
