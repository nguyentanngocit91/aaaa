...
class DanhSachMoiPendingLayout extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState createState() => _DanhSachMoiPendingLayoutState();
}

class _DanhSachMoiPendingLayoutState
    extends ConsumerState<DanhSachMoiPendingLayout>
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: TextFormField(
                        ...
                        decoration:
                            const InputDecoration(hintText: 'Số hợp đồng'),
                      ),
                    ),
                    const SizedBox(
                      width: 16,
                    ...
                    SizedBox(
                      ...
                      child: TextFormField(
                        ...
                        decoration:
                            const InputDecoration(hintText: 'Tên hợp đồng'),
                      ),
                    ),
                    const SizedBox(
                      width: 16,
                      ...
                      child: FilledButton.icon(
                          onPressed: () {
                            if (_soHopDongController.text.isNotEmpty ||
                                _tenHopDongController.text.isNotEmpty) {
                              List<String> dsfilter = [];
                              if (_soHopDongController.text.isNotEmpty) {
                                dsfilter.add(
                                    'sohopdong=${_soHopDongController.text}');
                              }
                              if (_tenHopDongController.text.isNotEmpty) {
                                dsfilter.add(
                                    'tenhopdong=${_tenHopDongController.text}');
                              }
                              ref
                                  .read(danhSachHopDongProvider.notifier)
                                  .changeFilter(filters: dsfilter);
                            } else {
                              showDialog(
                                context: context,
                                builder: (context) {
                                  return AlertDialog(
                                    ...
                                    content: const Text(
                                        'Vui lòng nhập Số Hợp Đồng hoặc Tên Hợp Đồng !'),
                                    actions: [
                                      FilledButton(
                                          onPressed: () {
            ...
            if (danhSachHopDongState.isLoading == true) ...[
              const Center(
                              ...
                              return DataRow.byIndex(
                                ...
                                cells: [
                                  ...
                                  DataCell(Text(item.hopdong![0]['sohopdong']
                                      .toString())),
                                  DataCell(Text(
                                      (item.hopdong![0]['tenhopdong'] != null)
                                          ? item.hopdong![0]['tenhopdong']
                                              .toString()
                                          : 'Uknow')),
                                  DataCell(Text(
                                      DateTime.parse(item.ngaytao.toString())
                                          .formatDateTime(
